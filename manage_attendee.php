...
$max_guests = 2; // Set your desired maximum number of guests here

// Count the number of existing attendees for the event
$attendee_count = $conn->query("SELECT COUNT(*) as total FROM attendees WHERE event_id = $event_id")->fetch_assoc()['total'];
	...
	<form action="" id="manage_attendee">
		...
		<div class="row">
			...
			<div class="col-md-6">
				...
				<?php if ($attendee_count >= $max_guests): ?>
				<div class="alert alert-danger" role="alert">
					Guest limit for this event has been reached. No more guests can be added.
				</div>
				<?php endif; ?>
			</div>
		</div>
...
<script>
	...
	$('#manage_attendee').submit(function(e) {
		...
		<?php if ($attendee_count >= $max_guests): ?>
			alert_toast('Guest limit for this event has been reached.', "danger");
			end_load();
		<?php else: ?>
			$.ajax({
